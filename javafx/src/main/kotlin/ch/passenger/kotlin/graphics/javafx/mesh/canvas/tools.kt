 ...
 */
abstract class CanvasTool<H:Any,V:Any, F:Any>(val canvas:FXMeshCanvas<H,V,F>, var id:Int) {
    ...
    companion object {
        init {
            CanvasToolFactory.tools add object : CanvasToolCreator() {
                override fun <H:Any, V:Any, F:Any> fits(vc: KClass<V>, ec: KClass<H>, fc: KClass<F>): Boolean = true
                override fun <H:Any, V:Any, F:Any> invoke(c: FXMeshCanvas<H, V, F>, id: Int): CanvasTool<H, V, F> = CanvasSelectTool(c, id)
            }
        }
        fun hi() {}
    }
...
class CanvasCreateEdgeTool<H : Any, V : Any, F : Any>(canvas: FXMeshCanvas<H, V, F>, id: Int) : CanvasTool<H, V, F>(canvas, id) {
    ...
    override fun start() {
        subclick = canvas.fromEvents(MouseEvent.MOUSE_CLICKED).doOnCompleted{submove?.unsubscribe(); submove=null; subclick=null}.subscribe {
            if(it.button == MouseButton.SECONDARY) cancel()
...
abstract class CanvasToolCreator() {
    abstract fun<H:Any,V:Any, F:Any> fits(vc:KClass<V>, ec:KClass<H>, fc:KClass<F>) : Boolean
    abstract fun<H:Any,V:Any, F:Any> invoke(c:FXMeshCanvas<H,V,F>, id:Int) : CanvasTool<H,V,F>
}

object CanvasToolFactory {
    val tools : MutableSet<CanvasToolCreator> = hashSetOf()

    fun<H:Any,V:Any, F:Any> invoke(vc:KClass<V>, ec:KClass<H>, fc:KClass<F>) : Iterable<CanvasToolCreator> = tools.filter { it.fits(vc, ec, fc) }
    init {
        CanvasSelectTool.hi()
    }
}